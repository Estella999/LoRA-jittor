...
class AdamW(jt.optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # translated from pytorch with equivalent functions
                exp_avg.assign(exp_avg.mul(beta1))
                exp_avg.add_(grad * (1.0-beta1))
                exp_avg_sq.assign(exp_avg_sq.mul(beta2))
                exp_avg_sq = exp_avg_sq + (1.0 - beta2) * grad * grad
                denom = exp_avg_sq.sqrt().add_(group["eps"])
                ...
                p.data = p.data + denom * (-step_size / exp_avg)
