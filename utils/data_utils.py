...
class BinLMOrderedIterator(object):
    ...
    def get_batch(self, i, bptt, eval_len):
        ...
        _input = jt.array(_inputs, dtype=jt.int64).contiguous()
        _target = jt.array(_targets, dtype=jt.int64).contiguous()
...
# custom dataset class for finetune data
class FT_Dataset(Dataset):
    def __init__(self, ft_filepath, batch_size, max_seq_length, max_eval_length = 0, joint_lm = False,
                 prefix_len = 0, infix_len = 0, prefix_cursor = 1000000, infix_cursor=2000000):
        super().__init__()
        self.ft_filepath = ft_filepath
        ...
        self.ft_samples = []
        with open(ft_filepath, 'r', encoding='utf-8') as f:
            for line in f:
                items = json.loads(line.strip())
                context = items['context']
                completion = items['completion']
                self.ft_samples.append([context, completion])

        self.num_samples = len(self.ft_samples)
        self.rng = random.Random(911)
        self.num_batches = int((self.num_samples + self.batch_size - 1) / self.batch_size)

    def __getitem__(self, k):
        if (k >= self.num_samples):
            k = self.rng.randint(0, self.num_batches - 1)

        sample = self.ft_samples[k]
        context = sample[0]
        completion = sample[1]
        ...
        output = {}
        output["id"] = jt.array(k, dtype=jt.int64)
